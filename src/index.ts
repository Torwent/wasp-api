...
server.use("/docs", swaggerUi.serve, swaggerUi.setup(swaggerDocs, swaggerUiOptions))
...
server.use(
	morgan(":remote-addr :user-agent :method :url :status :res[content-length] - :response-time ms")
)
server.use(cors())

//dynamically load routes from ./routes
fs.readdirSync(__dirname + "/routes/").forEach(async (file) => {
	const route = __dirname + "/routes/" + file
	const path = "/" + file.replace(/\.[jt]s/i, "") //regex replace .js or .ts
