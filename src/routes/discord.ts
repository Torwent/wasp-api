...
router.get("/:DISCORD_ID", async (req: Request, res: Response) => {
  ...
  if (!DISCORD_ID_REGEX.test(DISCORD_ID))
    return res
      .status(416)
      .send("Response code: 416 - That DISCORD_ID is not valid!")
...
router.get("/refresh/:DISCORD_ID", async (req: Request, res: Response) => {
  let { DISCORD_ID } = req.params
  ...
  switch (result) {
    ...
    case 417:
      return res
        .status(417)
        .send(
          "Response code: 417 - That DISCORD_ID is not in the discord server!"
        )
    ...
    case 200:
      return res
        .status(200)
        .send(
          "Response code: 200 - The user roles were refreshed in waspscripts.com"
        )
  }
})
